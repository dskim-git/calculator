# main.py
import math
import numpy as np
import pandas as pd
import streamlit as st
import plotly.graph_objects as go
import plotly.express as px

# ─────────────────────────────────────────────
# 기본 설정
# ─────────────────────────────────────────────
st.set_page_config(
    page_title="수학 계산기 & 세계인구 분석",
    page_icon="🧮",
    layout="centered",
)

st.title("🧮 수학 계산기 & 🌍 세계인구 분석 웹앱")
st.write("사칙연산, 모듈러, 지수, 로그, 다항함수 그래프, 연도별 세계인구 분석 기능을 제공합니다.")

# 분석에 사용할 연도 컬럼들
YEAR_COLUMNS = ["1970", "1980", "1990", "2000", "2010", "2015", "2020", "2022"]

# ─────────────────────────────────────────────
# 사이드바 메뉴
# ─────────────────────────────────────────────
menu = st.sidebar.selectbox(
    "기능 선택",
    [
        "사칙연산",
        "모듈러 연산 (a mod n)",
        "지수 연산 (a^b)",
        "로그 연산 (log_b(a))",
        "다항함수 그래프",
        "연도별 세계인구 분석",
    ],
)

# ─────────────────────────────────────────────
# 0. 공용 함수: 세계 인구 데이터 불러오기
# ─────────────────────────────────────────────
@st.cache_data
def load_population_data():
    """
    world_population.csv 파일을 불러옵니다.
    예상 컬럼:
      - code (ISO3 국가 코드)
      - Country
      - 1970, 1980, 1990, 2000, 2010, 2015, 2020, 2022
      - World Population Percentage
    """
    df = pd.read_csv("world_population.csv")
    # 컬럼 이름 양쪽 공백 제거
    df.columns = [c.strip() for c in df.columns]

    # 연도 및 비율 컬럼 숫자형으로 변환
    for col in YEAR_COLUMNS + ["World Population Percentage"]:
        if col in df.columns:
            df[col] = pd.to_numeric(df[col], errors="coerce")

    return df


# ─────────────────────────────────────────────
# 1️⃣ 사칙연산
# ─────────────────────────────────────────────
if menu == "사칙연산":
    st.subheader("사칙연산")

    op = st.selectbox(
        "연산 선택",
        ["덧셈 (+)", "뺄셈 (-)", "곱셈 (×)", "나눗셈 (÷)"],
    )

    a = st.number_input("첫 번째 수 a", value=0.0)
    b = st.number_input("두 번째 수 b", value=0.0)

    if st.button("계산하기", key="basic"):
        result = None
        latex = ""

        if op == "덧셈 (+)":
            result = a + b
            latex = f"{a} + {b} = {result}"

        elif op == "뺄셈 (-)":
            result = a - b
            latex = f"{a} - {b} = {result}"

        elif op == "곱셈 (×)":
            result = a * b
            latex = f"{a} \\times {b} = {result}"

        else:  # 나눗셈
            if b == 0:
                st.error("0으로 나눌 수 없습니다.")
            else:
                result = a / b
                latex = f"{a} \\div {b} = {result}"

        if result is not None:
            st.success("✅ 계산 결과")
            st.latex(latex)

# ─────────────────────────────────────────────
# 2️⃣ 모듈러 연산
# ─────────────────────────────────────────────
elif menu == "모듈러 연산 (a mod n)":
    st.subheader("모듈러 연산 (a mod n)")

    a = int(st.number_input("정수 a", value=10, step=1))
    n = int(st.number_input("양의 정수 n", value=3, min_value=1, step=1))

    if st.button("계산하기", key="mod"):
        result = a % n
        st.success("✅ 계산 결과")
        st.latex(f"{a} \\bmod {n} = {result}")

# ─────────────────────────────────────────────
# 3️⃣ 지수 연산
# ─────────────────────────────────────────────
elif menu == "지수 연산 (a^b)":
    st.subheader("지수 연산 (a^b)")

    a = st.number_input("밑 a", value=2.0)
    b = st.number_input("지수 b", value=3.0)

    if st.button("계산하기", key="power"):
        try:
            result = a ** b
            st.success("✅ 계산 결과")
            st.latex(f"{a}^{{{b}}} = {result}")
        except OverflowError:
            st.error("값이 너무 큽니다. 지수를 줄여보세요.")

# ─────────────────────────────────────────────
# 4️⃣ 로그 연산
# ─────────────────────────────────────────────
elif menu == "로그 연산 (log_b(a))":
    st.subheader("로그 연산 (log_b(a))")

    x = st.number_input("진수 a (>0)", value=8.0)
    b = st.number_input("밑 b (>0, b≠1)", value=2.0)

    if st.button("계산하기", key="log"):
        if x <= 0:
            st.error("a는 0보다 커야 합니다.")
        elif b <= 0 or b == 1:
            st.error("b는 0보다 크고 1이 아니어야 합니다.")
        else:
            result = math.log(x, b)
            st.success("✅ 계산 결과")
            st.latex(f"\\log_{{{b}}} {x} = {result}")

# ─────────────────────────────────────────────
# 5️⃣ 다항함수 그래프 (Plotly)
# ─────────────────────────────────────────────
elif menu == "다항함수 그래프":
    st.subheader("📈 다항함수 그래프 그리기")

    degree = st.selectbox("다항식 차수 선택", [1, 2, 3])

    if degree == 1:
        a = st.number_input("a (1차항 계수)", value=1.0)
        b = st.number_input("b (상수항)", value=0.0)
        st.latex(f"f(x) = {a}x + {b}")
        f = lambda x: a * x + b

    elif degree == 2:
        a = st.number_input("a (2차항 계수)", value=1.0)
        b = st.number_input("b (1차항 계수)", value=0.0)
        c = st.number_input("c (상수항)", value=0.0)
        st.latex(f"f(x) = {a}x^2 + {b}x + {c}")
        f = lambda x: a * x**2 + b * x + c

    else:  # 3차
        a = st.number_input("a (3차항 계수)", value=1.0)
        b = st.number_input("b (2차항 계수)", value=0.0)
        c = st.number_input("c (1차항 계수)", value=0.0)
        d = st.number_input("d (상수항)", value=0.0)
        st.latex(f"f(x) = {a}x^3 + {b}x^2 + {c}x + {d}")
        f = lambda x: a * x**3 + b * x**2 + c * x + d

    x_min, x_max = st.slider("x 범위 설정", -20, 20, (-5, 5))

    if st.button("그래프 그리기", key="poly_plot"):
        x = np.linspace(x_min, x_max, 400)
        y = f(x)

        fig = go.Figure()
        fig.add_trace(go.Scatter(x=x, y=y, mode="lines", name="f(x)"))

        fig.update_layout(
            title="다항함수 그래프",
            xaxis_title="x",
            yaxis_title="f(x)",
        )

        st.plotly_chart(fig, use_container_width=True)

# ─────────────────────────────────────────────
# 6️⃣ 연도별 세계인구 분석
# ─────────────────────────────────────────────
elif menu == "연도별 세계인구 분석":
    st.subheader("🌍 연도별 세계인구 분석")

    # 데이터 불러오기
    try:
        df_pop = load_population_data()
    except FileNotFoundError:
        st.error("world_population.csv 파일을 찾을 수 없습니다. main.py와 같은 폴더에 두고 다시 실행해 주세요.")
        st.stop()

    # 1) 연도별 인구 지도
    st.markdown("#### 1) 연도별 세계 인구 지도")

    year = st.selectbox(
        "연도를 선택하세요",
        options=YEAR_COLUMNS,
        format_func=lambda y: f"{y}년",
    )

    if st.button("선택 연도 인구 지도 보기", key="year_map"):
        if year not in df_pop.columns:
            st.error(f"{year} 컬럼을 찾을 수 없습니다. CSV 헤더를 확인해 주세요.")
        else:
            # 필요한 컬럼만 뽑아서 새 DataFrame 생성
            df_year = df_pop[["code", "Country", year]].copy()
            df_year = df_year.rename(columns={year: "Population"})

            # 혹시 모를 타입 문제 방지
            df_year["Population"] = pd.to_numeric(df_year["Population"], errors="coerce")

            fig_year = px.choropleth(
                df_year,
                locations="code",
                locationmode="ISO-3",
                color="Population",
                hover_name="Country",
                color_continuous_scale="Viridis",
                labels={"Population": f"Population {year}"},
            )
            fig_year.update_layout(
                title=f"{year}년 세계 인구 분포",
            )
            st.plotly_chart(fig_year, use_container_width=True)

    st.markdown("---")
    st.markdown("#### 2) 세계 인구 비율(%) 지도")

    if "World Population Percentage" not in df_pop.columns:
        st.error("'World Population Percentage' 열이 CSV에 존재하지 않습니다.")
    else:
        if st.button("세계 인구 비율 지도 보기", key="share_map"):
            df_share = df_pop[["code", "Country", "World Population Percentage"]].copy()
            df_share["World Population Percentage"] = pd.to_numeric(
                df_share["World Population Percentage"], errors="coerce"
            )

            fig_share = px.choropleth(
                df_share,
                locations="code",
                locationmode="ISO-3",
                color="World Population Percentage",
                hover_name="Country",
                color_continuous_scale="Plasma",
                labels={"World Population Percentage": "World Population %"},
            )
            fig_share.update_layout(
                title="세계 인구 비율(%) 분포",
            )
            st.plotly_chart(fig_share, use_container_width=True)

    with st.expander("📄 데이터 일부 미리보기"):
        st.dataframe(df_pop.head())

# ─────────────────────────────────────────────
# 사용 안내 공통
# ─────────────────────────────────────────────
with st.expander("ℹ️ 사용 안내"):
    st.markdown(
        """
        - **사칙연산**: 두 수에 대한 기본 연산  
        - **모듈러 연산**: a mod n (나머지 연산)  
        - **지수 연산**: \(a^b\) 계산  
        - **로그 연산**: \(\log_b a\) 계산  
        - **다항함수 그래프**: 1~3차 다항함수의 그래프를 Plotly로 시각화  
        - **연도별 세계인구 분석**:  
          - 선택한 연도의 인구수를 세계지도(choropleth)로 시각화  
          - 각 나라의 세계 인구 비율(%)을 색으로 표현  
        """
    )

if __name__ == "__main__":
    # streamlit run main.py 로 실행
    pass
