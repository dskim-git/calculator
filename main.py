# app.py
import math
import numpy as np
import streamlit as st
import plotly.graph_objects as go

# ─────────────────────────────────────────────
# 기본 설정
# ─────────────────────────────────────────────
st.set_page_config(
    page_title="수학 계산기 웹앱",
    page_icon="🧮",
    layout="centered",
)

st.title("🧮 수학 계산기 웹앱")
st.write("사칙연산, 모듈러, 지수, 로그, 다항함수 그래프까지 지원하는 계산기입니다.")

# ─────────────────────────────────────────────
# 사이드바 메뉴
# ─────────────────────────────────────────────
calc_type = st.sidebar.selectbox(
    "기능 선택",
    [
        "사칙연산",
        "모듈러 연산 (a mod n)",
        "지수 연산 (a^b)",
        "로그 연산 (log_b(a))",
        "다항함수 그래프",
    ],
)

# ─────────────────────────────────────────────
# 1️⃣ 사칙연산
# ─────────────────────────────────────────────
if calc_type == "사칙연산":
    st.subheader("사칙연산")

    op = st.selectbox(
        "연산 선택",
        ["덧셈 (+)", "뺄셈 (-)", "곱셈 (×)", "나눗셈 (÷)"],
    )

    a = st.number_input("첫 번째 수 a", value=0.0)
    b = st.number_input("두 번째 수 b", value=0.0)

    if st.button("계산하기", key="basic"):
        if op == "덧셈 (+)":
            result = a + b
            latex = f"{a} + {b} = {result}"

        elif op == "뺄셈 (-)":
            result = a - b
            latex = f"{a} - {b} = {result}"

        elif op == "곱셈 (×)":
            result = a * b
            latex = f"{a} \\times {b} = {result}"

        else:
            if b == 0:
                st.error("0으로 나눌 수 없습니다.")
                result = None
            else:
                result = a / b
                latex = f"{a} \\div {b} = {result}"

        if result is not None:
            st.success("✅ 계산 결과")
            st.latex(latex)

# ─────────────────────────────────────────────
# 2️⃣ 모듈러 연산
# ─────────────────────────────────────────────
elif calc_type == "모듈러 연산 (a mod n)":
    st.subheader("모듈러 연산 (a mod n)")

    a = int(st.number_input("정수 a", value=10, step=1))
    n = int(st.number_input("양의 정수 n", value=3, min_value=1, step=1))

    if st.button("계산하기", key="mod"):
        result = a % n
        st.success("✅ 계산 결과")
        st.latex(f"{a} \\bmod {n} = {result}")

# ─────────────────────────────────────────────
# 3️⃣ 지수 연산
# ─────────────────────────────────────────────
elif calc_type == "지수 연산 (a^b)":
    st.subheader("지수 연산 (a^b)")

    a = st.number_input("밑 a", value=2.0)
    b = st.number_input("지수 b", value=3.0)

    if st.button("계산하기", key="power"):
        try:
            result = a ** b
            st.success("✅ 계산 결과")
            st.latex(f"{a}^{{{b}}} = {result}")
        except OverflowError:
            st.error("값이 너무 큽니다.")

# ─────────────────────────────────────────────
# 4️⃣ 로그 연산
# ─────────────────────────────────────────────
elif calc_type == "로그 연산 (log_b(a))":
    st.subheader("로그 연산 (log_b(a))")

    x = st.number_input("진수 a (>0)", value=8.0)
    b = st.number_input("밑 b (>0, b≠1)", value=2.0)

    if st.button("계산하기", key="log"):
        if x <= 0:
            st.error("a는 0보다 커야 합니다.")
        elif b <= 0 or b == 1:
            st.error("b는 0보다 크고 1이 아니어야 합니다.")
        else:
            result = math.log(x, b)
            st.success("✅ 계산 결과")
            st.latex(f"\\log_{{{b}}} {x} = {result}")

# ─────────────────────────────────────────────
# 5️⃣ ✅ 다항함수 그래프 (Plotly)
# ─────────────────────────────────────────────
elif calc_type == "다항함수 그래프":
    st.subheader("📈 다항함수 그래프 그리기")

    degree = st.selectbox("다항식 차수 선택", [1, 2, 3])

    if degree == 1:
        a = st.number_input("a", value=1.0)
        b = st.number_input("b", value=0.0)
        st.latex(f"f(x) = {a}x + {b}")

        f = lambda x: a * x + b

    elif degree == 2:
        a = st.number_input("a", value=1.0)
        b = st.number_input("b", value=0.0)
        c = st.number_input("c", value=0.0)
        st.latex(f"f(x) = {a}x^2 + {b}x + {c}")

        f = lambda x: a * x**2 + b * x + c

    else:
        a = st.number_input("a", value=1.0)
        b = st.number_input("b", value=0.0)
        c = st.number_input("c", value=0.0)
        d = st.number_input("d", value=0.0)
        st.latex(f"f(x) = {a}x^3 + {b}x^2 + {c}x + {d}")

        f = lambda x: a * x**3 + b * x**2 + c * x + d

    x_min, x_max = st.slider("x 범위 설정", -10, 10, (-5, 5))

    if st.button("그래프 그리기", key="plot"):
        x = np.linspace(x_min, x_max, 400)
        y = f(x)

        fig = go.Figure()
        fig.add_trace(go.Scatter(x=x, y=y, mode="lines"))

        fig.update_layout(
            title="다항함수 그래프",
            xaxis_title="x",
            yaxis_title="f(x)",
        )

        st.plotly_chart(fig, use_container_width=True)

# ─────────────────────────────────────────────
# 사용 안내
# ─────────────────────────────────────────────
with st.expander("ℹ️ 사용 안내"):
    st.markdown("""
- **사칙연산**: 기본 계산  
- **모듈러 연산**: 나머지  
- **지수 연산**: 거듭제곱  
- **로그 연산**: 로그 계산  
- **다항함수 그래프**: 1~3차 함수 시각화  
""")
